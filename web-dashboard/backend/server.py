...
async def send_control(payload: ControlPayload):
    cmd = payload.command.upper()
    logger.info(f"Control command received: {cmd}")
    
    if cmd == "CALIBRATE":
        if state.serial_inst and state.serial_inst.is_open:
            state.serial_inst.write(b"CALIB\n")
        state.last_telemetry["score"] = 100
        state.last_telemetry["state"] = 1
        return {"status": "ok", "action": "calibration_started"}
        
    elif cmd == "TOGGLE_DEMO":
        state.demo_mode = not state.demo_mode
        if state.demo_mode:
            state.demo_state = 3 if state.demo_state == 1 else 1
        return {"status": "ok", "demo_mode": state.demo_mode, "demo_state": state.demo_state}
        
    elif cmd == "SET_FAULT":
        fault = payload.param.lower()
        if fault == "unbalance":
            state.demo_state = 3
        elif fault == "bearing":
            state.demo_state = 4
            ...
            state.demo_state = 1
        return {"status": "ok", "demo_state": state.demo_state}
